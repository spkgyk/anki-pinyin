...
class Scanner:
    ...
    def fetch_more_tokens(self):
        ...
        raise ScannerError(
            "while scanning for the next token", None, "found character %r that cannot start any token" % ch, self.get_mark()
        )
    ...
    def scan_directive_name(self, start_mark):
        ...
        if not length:
            raise ScannerError(
                "while scanning a directive", start_mark, "expected alphabetic or numeric character, but found %r" % ch, self.get_mark()
            )
    ...
    def scan_yaml_directive_value(self, start_mark):
        ...
        if self.peek() != ".":
            raise ScannerError(
                "while scanning a directive", start_mark, "expected a digit or '.', but found %r" % self.peek(), self.get_mark()
            )
        self.forward()
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        if ch not in "\0\r\n\x85\u2028\u2029":
            raise ScannerError(
                "while scanning a directive", start_mark, "expected a comment or a line break, but found %r" % ch, self.get_mark()
            )
    ...
    def scan_anchor(self, TokenClass):
        ...
        if not length:
            raise ScannerError(
                "while scanning an %s" % name, start_mark, "expected alphabetic or numeric character, but found %r" % ch, self.get_mark()
            )
        value = self.prefix(length)
        self.forward(length)
        ch = self.peek()
    ...
    def scan_block_scalar_ignored_line(self, start_mark):
        ...
        if ch not in "\0\r\n\x85\u2028\u2029":
            raise ScannerError(
                "while scanning a block scalar", start_mark, "expected a comment or a line break, but found %r" % ch, self.get_mark()
            )
        self.scan_line_break()
        ...
        while True:
            ...
            if not double and ch == "'" and self.peek(1) == "'":
                ...
                if ch in self.ESCAPE_REPLACEMENTS:
                    ...
                    for k in range(length):
                        if self.peek(k) not in "0123456789ABCDEFabcdef":
                            raise ScannerError(
                                ...
                                start_mark,
                                "expected escape sequence of %d hexadecimal numbers, but found %r" % (length, self.peek(k)),
                                self.get_mark(),
                            )
                    ...
                    raise ScannerError(
                        "while scanning a double-quoted scalar", start_mark, "found unknown escape character %r" % ch, self.get_mark()
                    )
            else:
                return chunks
    ...
    def scan_plain(self):
        ...
        while True:
            ...
            while True:
                ...
                if (
                    ch in "\0 \t\r\n\x85\u2028\u2029"
                    or (ch == ":" and self.peek(length + 1) in "\0 \t\r\n\x85\u2028\u2029" + (",[]{}" if self.flow_level else ""))
