...
class Emitter:
    ...
    def choose_scalar_style(self):
        ...
        if not self.event.style and self.event.implicit[0]:
            if not (self.simple_key_context and (self.analysis.empty or self.analysis.multiline)) and (
                self.flow_level and self.analysis.allow_flow_plain or (not self.flow_level and self.analysis.allow_block_plain)
            ):
                return ""
    ...
    def analyze_scalar(self, scalar):
        ...
        while index < len(scalar):
            ...
            if not (ch == "\n" or "\x20" <= ch <= "\x7E"):
                if (
                    ch == "\x85" or "\xA0" <= ch <= "\uD7FF" or "\uE000" <= ch <= "\uFFFD" or "\U00010000" <= ch < "\U0010ffff"
                ) and ch != "\uFEFF":
    ...
    def write_double_quoted(self, text, split=True):
        ...
        while end <= len(text):
            ...
            if (
                ch is None
                or ch in '"\\\x85\u2028\u2029\uFEFF'
                or not ("\x20" <= ch <= "\x7E" or (self.allow_unicode and ("\xA0" <= ch <= "\uD7FF" or "\uE000" <= ch <= "\uFFFD")))
            ):
                if start < end:
                    data = text[start:end]
                    self.column += len(data)
