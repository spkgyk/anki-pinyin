...
class Utterance:
            ...
            for token in self.tokens:
                ...
                if token.word == _CLITIC and prev_token is not None:
                    ...
                    if "%mor" in mor_gra_keys:
                        token_in_table.append(f"{prev_token.to_mor_tier()}~{token.to_mor_tier()}")
                    if "%gra" in mor_gra_keys:
                        token_in_table.append(f"{prev_token.to_gra_tier()} {token.to_gra_tier()}")
                else:
    ...
    def _repr_html_(self):
        ...
        # Row from words
        cells = [f'    <td style="text-align: left">{t.word}</td>\n' for t in self.tokens]
        html += "  <tr>\n" f"    <td>*{self.participant}:</td>\n" f"{''.join(cells)}" "  </tr>\n"

        # Row from %mor
        if "%mor" in self.tiers:
            cells = [f'    <td style="text-align: left">{t.to_mor_tier()}</td>\n' for t in self.tokens]
            html += "  <tr>\n" "    <td>%mor:</td>\n" f"{''.join(cells)}" "  </tr>\n"

        # Row from %gra
        if "%gra" in self.tiers:
            cells = [f'    <td style="text-align: left">{t.to_gra_tier()}</td>\n' for t in self.tokens]
            html += "  <tr>\n" "    <td>%gra:</td>\n" f"{''.join(cells)}" "  </tr>\n"
