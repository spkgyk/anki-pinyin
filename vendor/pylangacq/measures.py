...
def _get_ipsyn_for_file(tagged_sents) -> int:
    ...
    def N4(graph):
        ...
        for i in range(1, graph.n_nodes() - 1):
            ...
            if pos1 in {"pro:poss:det", "adj", "qn"} and (pos2.startswith("n:") or pos2 == "n"):
                scoring_board["N4"] += 1
    ...
    def N6(graph):
        ...
        for i in range(1, graph.n_nodes() - 2):
            ...
            if pos2 in {"pro:poss:det", "adj", "qn"} and (pos3.startswith("n:") or pos3 == "n") and (pos1 in {"v", "prep"}):
                scoring_board["N6"] += 1
                add_one_point_if_needed("N4")
    ...
    def N8(graph):
        ...
        for i in range(1, graph.n_nodes() - 2):
            ...
            if pos1 in {"pro:poss:det", "adj", "qn"} and (pos2.startswith("n:") or pos2 == "n") and (pos3 == "v"):
                scoring_board["N8"] += 1
                add_one_point_if_needed("N4")
    ...
    def N9(graph):
        ...
        for i in range(1, graph.n_nodes() - 2):
            ...
            if (pos1 in {"pro:poss:det", "adj", "qn"}) and (pos2 in {"adj", "qn"}) and (pos3.startswith("n:") or pos3 == "n"):
                scoring_board["N9"] += 1
                add_one_point_if_needed("N4")
    ...
    def V4(graph):
        ...
        for i in range(1, graph.n_nodes()):
            ...
            for dep, head in graph.edges().items():
                ...
                if graph.edge[dep][head]["rel"] == "SUBJ" and not graph.node[dep]["pos"].endswith("wh"):
                    subject = True
    ...
    def V6(graph):
        ...
        for i in range(1, graph.n_nodes()):
            ...
            if (pos == "aux" and not mor.startswith("wi")) or (lemma == "do" and pos == "mod"):
                scoring_board["V6"] += 1
    ...
    def S11(graph):
        ...
        for dep, head in graph.edges().items():
            ...
            if graph.edge[dep][head]["rel"] == "SUBJ" and graph.node[dep]["word"] != _CLITIC:
                subject_count += 1
    ...
    def S15(graph):
        ...
        number_of_verbs = sum(1 for i in range(1, graph.n_nodes()) if graph.node[i]["pos"] == "v")
