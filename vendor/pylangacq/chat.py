...
def _params_in_docstring(*params, class_method=True):
    ...
    def real_decorator(func):
        returns_none = f"Returns\n{'        ' if class_method else '    '}-------" not in func.__doc__
        if returns_none:
...
def _deprecate_warning(what, since_version, use_instead):
    ...
    """
    warnings.warn(
        f"'{what}' has been deprecated since PyLangAcq v{since_version}. " f"Please use {use_instead} instead.",
        FutureWarning,
    )
...
class Reader:
    ...
    def _parse_chat_strs(self, strs: List[str], file_paths: List[str], parallel: bool) -> None:
        if parallel:
            with cf.ProcessPoolExecutor() as executor:
                self._files = collections.deque(executor.map(self._parse_chat_str, strs, file_paths))
        else:
            self._files = collections.deque(self._parse_chat_str(s, f) for s, f in zip(strs, file_paths))
    ...
    def __getitem__(self, item):
        if type(item) == int:
            ...
            return self._get_reader_from_files(itertools.islice(self._files.copy(), start, stop, step))
        else:
            raise TypeError(f"Reader indices must be integers or slices, not {type(item)}")
    ...
    def filter(self, match: str = None, exclude: str = None) -> "pylangacq.Reader":
        ...
        reader._files = collections.deque(f for f in self._files if f.file_path in file_paths)
        return reader
    ...
    def utterances(self, participants=None, exclude=None, by_files=False) -> Union[List[Utterance], List[List[Utterance]]]:
    ...
    def tokens(
        ...
        """
        utterances = self.utterances(participants=participants, exclude=exclude, by_files=True)
    ...
    def words(
        ...
        result = [[[t.word for t in ts if t.word != _CLITIC] for ts in tss] for tss in tokens]
        return self._get_result_by_utterances_by_files(result, by_utterances, by_files)

    def _filter_utterances_by_participants(self, participants, exclude) -> List[List[Utterance]]:
        if participants and exclude:
            raise TypeError("participants and exclude cannot be specified at the same time: " f"{participants}, {exclude}")
        ...
        if participants is None:
            ...
            raise ValueError("participants must be one of {None, a string, an iterable of strings}: " f"{participants}")
        ...
        if exclude is None:
            ...
            raise ValueError("exclude must be one of {None, a string, an iterable of strings}: " f"{exclude}")

        return [[u for u in us if u.participant in ps] for us, ps in zip([f.utterances for f in self._files], participants)]
    ...
    @_params_in_docstring("by_files")
    def dates_of_recording(self, by_files=False) -> Union[Set[datetime.date], List[Set[datetime.date]]]:
    ...
    def ages(self, participant="CHI", months=False) -> Union[List[Tuple[int, int, int]], List[float]]:
    ...
    @_params_in_docstring("participants", "exclude", "by_files")
    def tagged_sents(self, participants=None, exclude=None, by_files=False) -> Union[List[List[Token]], List[List[List[Token]]]]:
    ...
    @_params_in_docstring("participants", "exclude", "by_files")
    def tagged_words(self, participants=None, exclude=None, by_files=False) -> Union[List[Token], List[List[Token]]]:
        ...
        """
        _deprecate_warning("tagged_words", "0.13.0", "the `.tokens()` method with by_utterances=False")
        utterances = self._filter_utterances_by_participants(participants, exclude)
    ...
    @_params_in_docstring("participants", "exclude", "by_files")
    def sents(self, participants=None, exclude=None, by_files=False) -> Union[List[List[str]], List[List[List[str]]]]:
        """Return the sents.
        ...
        """
        _deprecate_warning("words", "0.13.0", "the `.words()` method with by_utterances=True")
        utterances = self._filter_utterances_by_participants(participants, exclude)
        result_by_files = [[[t.word for t in u.tokens] for u in us] for us in utterances]
        if by_files:
            return result_by_files
        else:
            return self._flatten(list, result_by_files)
    ...
    def mlum(self, participant="CHI") -> List[float]:
        ...
        List[float]
        """
        return _get_mlum(self.tokens(participants=participant, by_utterances=True, by_files=True))
    ...
    def mluw(self, participant="CHI") -> List[float]:
        ...
        List[float]
        """
        return _get_mluw(self.words(participants=participant, by_utterances=True, by_files=True))
    ...
    def ttr(self, keep_case=True, participant="CHI") -> List[float]:
        ...
        List[float]
        """
        return _get_ttr(self.word_frequencies(keep_case=keep_case, participants=participant, by_files=True))
    ...
    def ipsyn(self, participant="CHI") -> List[int]:
        ...
        List[float]
        """
        return _get_ipsyn(self.tokens(participants=participant, by_utterances=True, by_files=True))
    ...
    def word_frequencies(
        ...
        result_by_files = [collections.Counter({k[0]: v for k, v in r.items()}) for r in result_by_files]
        if by_files:
            return result_by_files
        else:
            return self._flatten(collections.Counter, result_by_files)
    ...
    def from_strs(cls, strs: List[str], ids: List[str] = None, parallel: bool = True) -> "pylangacq.Reader":
        ...
        if ids is None:
            ...
            ids = list(ids)
        if len(strs) != len(ids):
            raise ValueError(f"strs and ids must have the same size: {len(strs)} and {len(ids)}")
        reader = cls()
    ...
    @staticmethod
    def _filter_file_paths(paths: List[str], match: str = None, exclude: str = None) -> List[str]:
        paths = list(paths)
        if match:
    ...
    def to_strs(self, tabular: bool = True) -> Generator[str, None, None]:
        ...
        for f in self._files:
            ...
            for key in header_keys:
                if key == "Languages":
                    str_for_file += f"@Languages:\t{' , '.join(f.header['Languages'])}\n"
                elif key == "Participants":
                    participants = f.header["Participants"]

                    parts = []
                    for code, demographics in participants.items():
                        parts.append(f"{code} {demographics['name']} {demographics['role']}")
    ...
    def _head_or_tail(self, slice_, participants, exclude):
        us = self.utterances(participants=participants, exclude=exclude, by_files=False).__getitem__(slice_)
        return _list(us)
    ...
    def to_chat(
        ...
        if not is_dir:
            ...
            if not basename and dir_.endswith(os.sep):
                ...
                raise ValueError(f"You've passed in {dir_} as `path`, but it doesn't look like " "a path to a file.")
            filenames = [basename]
        else:
            dir_ = path
            ...
            if len(filenames) != len(self._files):
                raise ValueError(f"There are {len(self._files)} CHAT files to create, " f"but you've provided {len(filenames)} filenames.")
        if dir_:
    ...
    def _get_utterances(self, all_tiers: Iterable[Dict[str, str]]) -> List[Utterance]:
        ...
        for tiermarker_to_line in all_tiers:
            ...
            # %gra tier
            gra_items = tiermarker_to_line["%gra"].split() if "%gra" in tiermarker_to_line else []

            if mor_items and gra_items and (len(mor_items) != len(gra_items)):
                raise ValueError(f"cannot align the %mor and %gra tiers:\n{tiermarker_to_line}")
...
def read_chat(
    ...
    if path_lower.endswith(".zip"):
        ...
        raise ValueError("path is not one of the accepted choices of " f"{{.zip file, local directory, .cha file}}: {path}")
...
class _HTTPSession(requests.Session):
    def __init__(self, max_retries: int = 10, backoff_factor: float = 0.1, timeout: int = 10):
        super().__init__()
