...
def _get_utterance(unparsed_sent, segment_kwargs, pos_tag_kwargs, participant) -> Utterance:
    if participant is not None:
        ...
        raise TypeError("Utterance must be either a string or " f"a tuple of (participant, utterance): {unparsed_sent}")
    participant = str(participant)

    if not unparsed_sent:
        return Utterance(participant=participant, tokens=[], time_marks=None, tiers={participant: ""})
    words, tags, jps = _parse_text(unparsed_sent, segment_kwargs, pos_tag_kwargs)

    tokens = [Token(word, pos, jp, None, None, None) for word, pos, jp in zip(words, tags, jps)]
    ...
    return Utterance(
        ...
        tiers={
            ...
            "%mor": " ".join(
                word if pos == "PUNCT" or pos[0].upper() not in _ASCII_UPPERCASE else f"{pos}|{jp or ''}"
...
def parse_text(
    ...
    *,
    ...
    if parallel:
        ...
        utterances = [_get_utterance(sent, segment_kwargs, pos_tag_kwargs, participant) for sent in data]
