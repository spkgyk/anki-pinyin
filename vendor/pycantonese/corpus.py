...
class CHATReader(Reader):
    ...
    def ipsyn(self):
        ...
        raise NotImplementedError("The upstream `ipsyn` method works for English only. " "There isn't yet a Cantonese version of IPSyn.")
    ...
    def jyutping(
        ...
        result = [
            [[tagged_word.jyutping for tagged_word in tagged_sent] for tagged_sent in tagged_sents_for_file]
    ...
    def jyutping_sents(self, participants=None, exclude=None, by_files=False):
        _deprecate("jyutping_sents", "jyutping with by_utterances=True", "3.2.0", "4.0.0")
    ...
    def jyutpings(self, participants=None, exclude=None, by_utterances=False, by_files=False):
    ...
    def characters(
        ...
        result = [[self._get_chars_from_sent(sent) for sent in sents_for_file] for sents_for_file in sents]
        if by_files and by_utterances:
            pass
        elif by_files and not by_utterances:
            result = [self._flatten(list, f) for f in result]
    ...
    def character_sents(self, participants=None, exclude=None, by_files=False):
        _deprecate("character_sents", "characters with by_utterances=True", "3.2.0", "4.0.0")
...
def read_chat(path: str, match: str = None, exclude: str = None, encoding: str = _ENCODING) -> CHATReader:
    ...
    :class:`~pycantonese.CHATReader`
    """
    return pylangacq_read_chat(path, match=match, exclude=exclude, encoding=encoding, cls=CHATReader)
