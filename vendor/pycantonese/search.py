...
def _perform_search(
    ...
    if not (type(words_left) == type(words_right) == type(sents_left) == type(sents_right) == int):
    ...
    if tone is None:
        ...
        raise ValueError(f"tone is used, but it's not one of {{1, 2, 3, 4, 5, 6}}: {tone}")
    else:
        tone = str(tone)
    ...
    if jp_search:
        ...
        if final and (nucleus or coda):
            raise ValueError("final cannot be used together with either nucleus or coda (or both)")
        if jyutping and (onset or final or nucleus or coda or tone):
            raise ValueError("jyutping cannot be used together with other Jyutping elements")
