...
def from_bytes(
    ...
    for encoding_iana in prioritized_encodings + IANA_SUPPORTED:
        ...
        if multi_byte_bonus:
            logger.log(
                TRACE,
                "Code page %s is a multi byte encoding table and it appear that at least one character " "was encoded using n-bytes.",
                encoding_iana,
            )
        ...
        if mean_mess_ratio >= threshold or early_stop_count >= max_chunk_gave_up:
            ...
            logger.log(
                TRACE,
                "%s was excluded because of initial chaos probing. Gave up %i time(s). " "Computed mean chaos is %f %%.",
                encoding_iana,
                early_stop_count,
                round(mean_mess_ratio * 100, ndigits=3),
            )
            # Preparing those fallbacks in case we got nothing.
            if enable_fallback and encoding_iana in ["ascii", "utf_8", specified_encoding] and not lazy_str_hard_failure:
                fallback_entry = CharsetMatch(sequences, encoding_iana, threshold, False, [], decoded_payload)
        ...
        if encoding_iana in [specified_encoding, "ascii", "utf_8"] and mean_mess_ratio < 0.1:
            logger.debug("Encoding detection: %s is most likely the one.", encoding_iana)
            if explain:
                logger.removeHandler(explain_handler)
        ...
        if encoding_iana == sig_encoding:
            logger.debug(
                "Encoding detection: %s is most likely the one as we detected a BOM or SIG within " "the beginning of the sequence.",
                encoding_iana,
            )
            if explain:
                logger.removeHandler(explain_handler)
