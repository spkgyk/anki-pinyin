...
class HTTPResponse(BaseHTTPResponse):
    ...
    def _fp_read(self, amt: int | None = None) -> bytes:
        ...
        assert self._fp
        c_int_max = 2**31 - 1
        if ((amt and amt > c_int_max) or (self.length_remaining and self.length_remaining > c_int_max)) and (
            util.IS_PYOPENSSL or sys.version_info < (3, 10)
        ):
            buffer = io.BytesIO()
    ...
    def stream(self, amt: int | None = 2**16, decode_content: bool | None = None) -> typing.Generator[bytes, None, None]:
        """
    ...
    def read_chunked(self, amt: int | None = None, decode_content: bool | None = None) -> typing.Generator[bytes, None, None]:
        """
