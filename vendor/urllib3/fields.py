...
def format_header_param(name: str, value: _TYPE_FIELD_VALUE) -> str:
    ...
    warnings.warn(
        "'format_header_param' has been renamed to " "'format_multipart_header_param'. The old name will be " "removed in urllib3 v2.1.0.",
        DeprecationWarning,
        stacklevel=2,
    )
    return format_multipart_header_param(name, value)
...
class RequestField:
    ...
    def from_tuples(
        ...
        if isinstance(value, tuple):
            if len(value) == 3:
                filename, data, content_type = typing.cast(typing.Tuple[str, _TYPE_FIELD_VALUE, str], value)
            else:
